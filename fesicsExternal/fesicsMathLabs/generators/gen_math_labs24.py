...
LABS = [
    ("h2_de_morgan_lab_CommonMath2_SetLogic_Ep04.html",
     "집합 실험실 — 여집합을 씌우면 ∪는 어떻게 될까?",
     "집합 실험실 — 여집합을 씌우면 ∪는 어떻게 될까?",
     "1부터 12까지에서 두 집합을 만들어 여집합 연산 결과를 원소 단위로 대조한다.",
     LAB_DEM),
    ("h2_proposition_converse_lab_CommonMath2_SetLogic_Ep06.html",
     "명제 실험실 — 참인 명제의 역도 참일까?",
     "명제 실험실 — 참인 명제의 역도 참일까?",
     "1부터 24까지 모든 수를 확인해 원명제·역·이·대우의 참거짓을 판정하고 반례를 찾는다.",
     LAB_PROP),
    ("h2_function_test_lab_CommonMath2_Func_Ep01.html",
     "함수 판정 실험실 — 그래프면 다 함수일까?",
     "함수 판정 실험실 — 그래프면 다 함수일까?",
     "세로선을 옮기며 그래프와 만나는 점의 개수를 세어 함수와 일대일 함수를 판정한다.",
     LAB_FUNC),
    ("h2_composite_function_lab_CommonMath2_Func_Ep03.html",
     "합성함수 실험실 — 넣는 순서를 바꾸면?",
     "합성함수 실험실 — 넣는 순서를 바꾸면?",
     "두 일차함수를 서로 다른 순서로 합성해 식과 값을 비교한다.",
     LAB_COMP),
    ("h2_inverse_function_lab_CommonMath2_Func_Ep05.html",
     "역함수 실험실 — f⁻¹은 1/f 일까?",
     "역함수 실험실 — f⁻¹은 1/f 일까?",
